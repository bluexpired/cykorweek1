...
void prologue(char *func_name)
{
    ...
    char sfp_info[50]; // SFP 설명을 저장할 문자열 버퍼
    snprintf(sfp_info, sizeof(sfp_info), "%s SFP", func_name); // 함수 이름 포함하여 저장

    push(FP, sfp_info); // SFP 정보 푸시
...
void epilogue(void)
{
    SP = FP; // SP 를 FP 위치로 갱신
    FP = call_stack[FP]; // SFP로 이전 FP 복원
    pop(); // 전함수 sfp pop
    pop(); // return 주소 pop
...
void pop(void)
{
    ...
    call_stack[SP] = 0;  // 데이터 삭제 
    stack_info[SP][0] = ' ';  // 스택 설명 초기화
